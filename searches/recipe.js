/**
 * An attempt at understanding 'search'.  I failed.
 */
module.exports = {
   key: 'recipe',
...
//         const url = 'http://57b20fb546b57d1100a3c405.mockapi.io/api/recipes';
         ...
         const options = {
            params: {
               userid: bundle.inputData.style
            }
         };

         let url = 'https://' + bundle.authData.siteName + '.dozuki.com/api/2.0/work_log';

         return z.request(url, options)
          .then(response => {
             for (let x in response.json.results) {
                response.json.results[x].id = response.json.results[x].entryid;
             }
